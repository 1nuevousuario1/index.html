    ...
    <div className="max-w-xl mx-auto px-6 py-16 text-center" data-testid="checkout-success-page">
      ...
      {status === "paid" && (
        <>
          ...
          <p className="text-[#4B5563] font-nunito mt-1 text-sm">
            Recibirás un mensaje del equipo de Mundo Infantil para coordinar el envío.
          </p>
          <div className="flex gap-3 justify-center mt-8">
            <Link to="/catalogo" className="mi-btn-primary">Seguir comprando</Link>
            <Link to="/" className="mi-btn-yellow">Ir al inicio</Link>
          </div>
        </>
      )}
      {status === "expired" && (
      ...
      {status === "error" && (
        <>
          ...
          <p className="text-[#4B5563] mt-2 font-nunito">
            Si el cargo se completó, te contactaremos para confirmar tu pedido.
          </p>
          <Link to="/" className="mi-btn-primary mt-6 inline-block">Volver al inicio</Link>
        </>
      )}
    </div>
  );
